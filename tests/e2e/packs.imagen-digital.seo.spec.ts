...
test.describe('E2E/SEO: /es/packs/pack-imagen-digital (Pack Imagen Digital)', () => {
  const URL = '/es/packs/pack-imagen-digital';
  ...
  test('Debe cumplir con los requisitos SEO básicos', async ({ page }) => {
    ...
    // 4. Hero Badge
    const heroBadge = heroSection.getByText('Ingeniería Web');
    ...
    await expect(heroCTA).toHaveAttribute('href', '/es/formulario-contratacion/');

    // 5. Hero CTA secundario: "Ver mi perfil actual"
    const demoCTA = page.locator('#hero a').getByText('Ver mi perfil actual');
    await expect(demoCTA).toHaveAttribute('href', /#auditoria/);
    ...
    await expect(painReviews).toContainText('No salgo en el mapa');

    // 9. Interlinking a gestion-reputacion-online (servicio suelto)
    const linkToReputacion = page.locator('#desafios').locator('a[href*="/servicios/gestion-reputacion-online"]');
    ...
    expect(scriptContent).toContain('"description":"Servicio mensual de gestión de reputación y mantenimiento web"');
    ...
    const ctaFinal = page.locator('#cta-final').getByText('Proteger mi negocio local');
