...
test.describe('E2E/SEO: Blog Hub (ES)', () => {
  ...
  test('🔥 Debe renderizar categorías con iconos y tooltips', async ({ page }) => {
    const categories = page.locator('a[data-testid="category-filter"]');
    ...
    await expect(firstCat).toHaveAttribute('href', /.*\/es\/blog$/);
    await expect(firstCat).toHaveAttribute('title', 'Ver todo');

    // Verificar categoría WPO con tooltip
    const wpoCat = page.locator('a[data-testid="category-filter"][href*="wpo"]');
    await expect(wpoCat).toHaveAttribute('title', 'Optimización de velocidad y carga');
  });

  test('🔥 Debe mostrar posts iniciales (No vacío)', async ({ page }) => {
    // Ya tenemos posts reales, por lo que el empty state debe estar oculto
...
test.describe('E2E/SEO: Blog Hub (EN)', () => {
  ...
  test('🔥 Debe tener categorías en inglés', async ({ page }) => {
    ...
    // Verificar primer categoría (All)
    const firstCat = categories.nth(0);
    await expect(firstCat).toHaveAttribute('href', /.*\/en\/blog$/);
    await expect(firstCat).toHaveAttribute('title', 'View All');
  });
});